...
object SplunkLocationTrendQuery extends QueryTraits {
  ...
  /**
   * Calculate the total number of occurrences of each location, regardless of the query performed.
   * 
   ...
   * @return List of locations with the total number of occurrences for each.
   */
  private def queryBase(t: Int, b: Int, e: Int, p: SensisQueryElement, r: Array[String]): IQueryable[SensisQueryElement] = {
    ...
    for (i <- b to e) {
      ...
      for (c <- queryCan) {
        val tmp_c = "location" $eq c.getProperty[String]("location")
