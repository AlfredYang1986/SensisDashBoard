...
object SplunkResultHandle extends ResultHandle {
  ...
  def saveEndPoint(functionName: String) {
    val queryData = from db () in "splunkEndPoints" where ("function_name" -> functionName) select (x => x)
    if (queryData.empty) {
      _data_connection.getCollection("splunkEndPoints") += MongoDBObject("function_name" -> functionName)
    }
  }

  def saveUserKey(userKey: String) {
    val queryData = from db () in "splunkUserKeys" where ("user_key" -> userKey) select (x => x)
    if (queryData.empty) {
      _data_connection.getCollection("splunkUserKeys") += MongoDBObject("user_key" -> userKey)
    }
  }
}
