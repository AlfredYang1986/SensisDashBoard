...
object SplunkResultHandle extends ResultHandle {
    ...
    (scala.xml.XML.loadString("<root>" + result.substring(result.indexOf("?>") + 2) + "</root>") \\ "result" \ "field").map { field =>
      	...
      	if (k == "_raw") {
      		...
      		if (method_name.equals("search"))
      			addDistinctQueryAndLocation(mp)
      	}
      }
  }
  ...
  def addDistinctQueryAndLocation(argsMap: Map[String, String]) = {
	  ...
	  def getUserQuery(): String = argsMap.get("query") match {
		  	case Some(e) => val s = URLArgDecoding.decoding(e.toString()); println(s); s
		  	case none => ""
      	  }

	  def getLocation(): String = argsMap.get("location") match {
        	case Some(e) => 
	  ...
	  val data = from db () in "splunk_query_data" where ("query" $eq query, "location" $eq location) select (x => x)
	  if ((query != "") && (data.empty))
		  _data_connection.getCollection("splunk_query_data") += MongoDBObject("query" -> query, "location" -> location, "occurances" -> 1)
	  else if ((query != "") && !(data.empty))
